...
fn render_error_response(response: ErrorResponse, server_hostname: &str) -> Option<String> {
    let guidance = match response.error_type.as_deref() {
        Some("account_required") => Some(format!(
            "Run `termlog auth` to link this CLI to your {server_hostname} account."
        )),

        Some("upload_limit_reached") => {
            Some("Run `termlog auth` and follow the link to upload more.".to_owned())
        }
        ...
        assert_eq!(
            ...
            Some(
                "This action requires an account\n\n\
                 Run `termlog auth` to link this CLI to your example.com account."
                    .to_owned()
            )
        );
    }
        ...
        assert_eq!(
            ...
            Some(
                "Anonymous upload limit reached\n\n\
                 Run `termlog auth` and follow the link to upload more."
                    .to_owned()
            )
        );
